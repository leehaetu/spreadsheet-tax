...
| Multiple foreign-property annual adjustments | `POST /api/workflows/run` with one `foreignProperty[]` entry per configured country (`UI_WIRED`) | Foreign-property annual adapter exists | The application holds a single foreign business ID field. Confirm against real HMRC business discovery whether all configured countries belong under that business for the customer; do not assume. |
...
| HMRC calculation | `calc`, `calc_list`, `final_calc` workflows | HMRC calculation adapters exist | UI correctly avoids inventing a tax calculation. Readable calculation-detail presentation is incomplete. |
...
| Expired authorisation, HMRC outage and duplicate prevention | Some backend errors and submission-integrity checks exist | HMRC HTTP status/error codes are returned on relevant paths | Consolidated customer recovery UI is not yet fully implemented. No error may be shown as success. |
