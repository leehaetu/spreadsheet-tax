(async function initialiseQuarterlySources() {
  ...
  try {
    ...
    if (!sources.length) {
      root.innerHTML = '<div class="empty-setup"><strong>No income sources retrieved</strong><span>Connect HMRC and retrieve your registered businesses before preparing an update.</span><a class="btn btn-primary" href="/onboarding">Connect HMRC</a></div>';
      return;
    }
    ...
    root.querySelectorAll('.quarterly-source-row').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        note.hidden = false;
        note.textContent = 'Source selected. Upload a digitally linked spreadsheet or choose a clearly labelled fictional sample in preview mode.';
